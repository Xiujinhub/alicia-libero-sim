...
# 来源：真机 TCP 标定（用户提供）。四个工具装在同一片法兰上，画面上每个工具画一根箭头
# （见 ``arm_core.draw_tool_arrows``）：
#   * 起点 = **安装点** = 法兰平面上的 ``(vx, vy, 0)``（四个工具是并排装的，侧偏就体现在这儿）；
#   * 方向 = **法兰 +z（工具轴）**，长度 = ``vz``（该工具的轴向长度）；
#   * 于是四根箭头**互相平行、都平行于末端姿态的工具轴**，箭头尖正好落在各自 TCP 上。
...
# 这块法兰盘自己的长度，真机上装的是下面这几种长杆工具（29.5 / 24.2 / 24.4 / 43.2 cm）。
...
TOOLS: dict[str, dict] = {
    ...
    "刷子": dict(v=(0.09085388920106247, 0.03102611498131784, 0.42145140673414233),
                 rgba=(0.35, 0.80, 0.30, 1.0)),        # 绿
}
# 画面里箭头的默认杆半径[m]（箭头头部 = 2.2 倍杆半径）。四种颜色都避开了机械臂自身的
