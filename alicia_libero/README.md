...
| t5 细高瓶→木托盘 | ✅ | 348 | **合计** | **8 / 9** | |

t2 / t3 / t6 另测过鲁棒性：物体位置抖动 ±10mm × 5 次均为 **5/5 通过**
（`tests\bench_tasks.py 2 5 0.01`、`tests\bench_tasks.py 3 5 0.01`、`tests\bench_tasks.py 6 5 0.01`）。
...
* **t7**：抓的是布丁盖，下降时杯体先碰到罐子**侧面**，被"接触判据"当成到位 →
